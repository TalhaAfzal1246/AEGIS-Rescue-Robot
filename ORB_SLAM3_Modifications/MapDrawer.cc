...
#include "MapDrawer.h"
std::vector<std::vector<float>> global_aegis_hazards;
std::mutex global_hazards_mutex;
#include "MapPoint.h"
#include "KeyFrame.h"
...
// --- AEGIS 3D HOLO-MARKERS ---
// --- AEGIS 3D HOLO-MARKERS ---
    global_hazards_mutex.lock();
    for (size_t i = 0; i < global_aegis_hazards.size(); i++) {
        float hx = global_aegis_hazards[i][0];
        float hy = global_aegis_hazards[i][1];
        float hz = global_aegis_hazards[i][2];
        int type = (int)global_aegis_hazards[i][3];

        glPushMatrix();
        glTranslatef(hx, hy, hz); // Move to the exact 3D location
        
        // Color code the markers
//        if (type == 1) glColor3f(0.0f, 1.0f, 0.0f);      // Green = Conscious
        if (type == 1) glColor3f(1.0f, 1.0f, 0.0f);
        else if (type == 2) glColor3f(1.0f, 0.0f, 0.0f); // Red = Unconscious
        else if (type == 3) glColor3f(1.0f, 0.5f, 0.0f); // Orange = Fire
        
        // Raw Native OpenGL Wireframe Box
        glLineWidth(5.0f); // Thick, highly visible lines
        float w = 0.15f;   // 15cm size
        glBegin(GL_LINES);
        // Bottom Square
